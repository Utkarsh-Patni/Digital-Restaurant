...
<body>
  ...
  if ($payment == true) {
    echo "<h3> Thank you for paying bill.</h3>";
  } else {
  ...
  ?>
    ...
    <div class="section white-text" style="background: #B35458;">
      ...
      foreach ($arr as $key) {
      ?>
        <?php $amount=$key['SUM(food.price)']; ?>

       <?php if($amount==null){
      echo "<h5 class='center'> You have not ordered anything yet.</h5>";
    }

 else{ ?>
        <h5 class=center><?php echo "The Amount Payable = " . $key['SUM(food.price)']; ?> Rs. </h5>
    <?php } 
      }
    }

    $sql2 ="INSERT INTO `useramt` (`user_name`, `amount`) VALUES (?,?)";

 
if($amount!=null){
    $query2  = $pdoconn->prepare($sql2);
    $result2 = $query2->execute([$user_name,$amount]);
  } 
  ?>
